...
async function activate(context) {
  ...
  async function initializeGroupsAndColors() {
    ...
    // Ensure Diff group exists with a special color
    if (!groupColors["Diff"]) {
      groupColors["Diff"] = "#ff6b6b"; // Red color for diff bookmarks
    }
  ...
  class BookmarkItem extends vscode.TreeItem {
    constructor(bookmark) {
      ...
      const prefix = bookmark.group === "Diff" ? "🔀 " : "";
      super(
        `${prefix}${fileName}:${bookmark.line + 1} ${bookmark.content}`,
        vscode.TreeItemCollapsibleState.None
      );
      this.bookmark = bookmark;
      this.contextValue = "bookmarkItem";
      this.tooltip = `${u.fsPath} (Line ${bookmark.line + 1})${
        bookmark.group === "Diff" ? " - Diff Mode" : ""
      }`;
      this.command = {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("bm.toggleBookmark", async () => {
      ...
      const isDiff =
        ...
        (e.document.uri.scheme === "file" &&
          vscode.window.tabGroups.activeTabGroup.activeTab?.input?.original);

      // Force Diff group for diff mode bookmarks
      let bookmarkGroup = activeGroup;
      if (isDiff) {
        bookmarkGroup = "Diff";
      }
      ...
      if (!groups[bookmarkGroup]) {
        ...
        bookmarkGroup = names[0] || "Default";
      }

      const bms = getBookmarks();
      const idx = bms.findIndex(
        (b) => b.uri === uri && b.line === line && b.group === bookmarkGroup
      );
      ...
      if (idx >= 0) {
        ...
        bms.push({
          ...
          line,
          content,
          group: bookmarkGroup, // <-- Use bookmarkGroup, not activeGroup
          isDiff,
        });
        vscode.window.showInformationMessage(
          `Bookmark added to ${bookmarkGroup} group` // <-- Use bookmarkGroup
        );
      }
    ...
    vscode.commands.registerCommand("bm.nextBookmark", async () => {
      ...
      const sorted = getBookmarks()
        .filter((b) => {
          // In diff mode, only show diff bookmarks
          // In regular mode, exclude diff bookmarks
          if (isInDiff) {
            return (
              b.group === "Diff" && b.fsPath === editor.document.uri.fsPath
            );
          } else {
            return b.group === activeGroup && !b.isDiff;
          }
        })
        .sort((a, b) =>
          a.fsPath !== b.fsPath
      ...
      if (!sorted.length) {
        return vscode.window.showInformationMessage(
          isInDiff
            ? "No diff bookmarks in this file"
            : `No bookmarks in ${activeGroup} group`
        );
      }
      ...
      await vscode.commands.executeCommand("bm.openBookmark", {
        bookmark: nextBm,
      });
    }),
